...
import com.jicstech.orumesh.*;
import com.jicstech.orumesh.controllers.*;
import com.jicstech.orumesh.network.*;
...
import com.jicstech.orumesh.conf.Configuration.DefaultConfSettings;
import com.jicstech.orumesh.hash.Curl;
import com.jicstech.orumesh.hash.PearlDiver;
import com.jicstech.orumesh.model.Hash;
import com.jicstech.orumesh.service.dto.AbstractResponse;
import com.jicstech.orumesh.service.dto.AccessLimitedResponse;
import com.jicstech.orumesh.service.dto.AddedNeighborsResponse;
import com.jicstech.orumesh.service.dto.AttachToTangleResponse;
import com.jicstech.orumesh.service.dto.ErrorResponse;
import com.jicstech.orumesh.service.dto.ExceptionResponse;
import com.jicstech.orumesh.service.dto.FindTransactionsResponse;
import com.jicstech.orumesh.service.dto.GetBalancesResponse;
import com.jicstech.orumesh.service.dto.GetInclusionStatesResponse;
import com.jicstech.orumesh.service.dto.GetNeighborsResponse;
import com.jicstech.orumesh.service.dto.GetNodeInfoResponse;
import com.jicstech.orumesh.service.dto.GetTipsResponse;
import com.jicstech.orumesh.service.dto.GetTransactionsToApproveResponse;
import com.jicstech.orumesh.service.dto.GetTrytesResponse;
import com.jicstech.orumesh.service.dto.RemoveNeighborsResponse;
import com.jicstech.orumesh.utils.Converter;
import com.jicstech.orumesh.utils.MapIdentityManager;
...
public class API {
    ...
    private OruMesh instance;

    public API(OruMesh instance, IXI ixi) {
    ...
    private AbstractResponse process(final String requestString, InetSocketAddress sourceAddress) throws UnsupportedEncodingException {
        ...
        try {
            ...
            switch (command) {
                ...
                case "getNodeInfo": {
                    String name = instance.configuration.booling(Configuration.DefaultConfSettings.TESTNET) ? Driver.TESTNET_NAME : Driver.MAINNET_NAME;
                    return GetNodeInfoResponse.create(name, Driver.VERSION, Runtime.getRuntime().availableProcessors(),
    ...
    private AbstractResponse getTrytesStatement(List<String> hashes) throws Exception {
        ...
        for (final String hash : hashes) {
            final TransactionViewModel transactionViewModel = TransactionViewModel.fromHash(instance.mesh, new Hash(hash));
    ...
    public boolean storeTransactionStatement(final List<String> trys) throws Exception {
        for (final String trytes : trys) {
            ...
            if(transactionViewModel.store(instance.mesh)) {
                transactionViewModel.setArrivalTime(System.currentTimeMillis() / 1000L);
                instance.transactionValidator.updateStatus(transactionViewModel);
                transactionViewModel.updateSender("local");
                transactionViewModel.update(instance.mesh, "sender");
            }
        }
        return true;
    }
    ...
    private AbstractResponse getNewInclusionStateStatement(final List<String> trans, final List<String> tps) throws Exception {
        ...
        int[] tipsIndex = tips.stream().map(hash -> TransactionViewModel.quietFromHash(instance.mesh, hash))
        ...
        if(minTipsIndex > 0) {
            ...
            transactions.stream().map(hash -> TransactionViewModel.quietFromHash(instance.mesh, hash)).forEach(transaction -> {
        ...
        for (final Hash tip : tips) {
            TransactionViewModel transactionViewModel = TransactionViewModel.fromHash(instance.mesh, tip);
        ...
        for(int i = 0; i < inclusionStates.length; i++) {
            if(inclusionStates[i] == 0) {
                TransactionViewModel transactionViewModel = TransactionViewModel.fromHash(instance.mesh, transactions.get(i));
    ...
    private boolean exhaustiveSearchWithinIndex(Queue<Hash> nonAnalyzedTransactions, Set<Hash> analyzedTips, List<Hash> transactions, int[] inclusionStates, int count, int index) throws Exception {
        ...
        while ((pointer = nonAnalyzedTransactions.poll()) != null) {
            ...
            if (analyzedTips.add(pointer)) {
                ...
                final TransactionViewModel transactionViewModel = TransactionViewModel.fromHash(instance.mesh, pointer);
    ...
    private AbstractResponse findTransactionStatement(final Map<String, Object> request) throws Exception {
        ...
        if (request.containsKey("bundles")) {
            for (final String bundle : (List<String>) request.get("bundles")) {
                ...
                bundlesTransactions.addAll(BundleViewModel.load(instance.mesh, new Hash(bundle)).getHashes());
            }
        }
        ...
        if (request.containsKey("addresses")) {
            ...
            for (final String address : addresses) {
                ...
                addressesTransactions.addAll(AddressViewModel.load(instance.mesh, new Hash(address)).getHashes());
            }
        }
        ...
        if (request.containsKey("tags")) {
            for (String tag : (List<String>) request.get("tags")) {
                ...
                tagsTransactions.addAll(TagViewModel.load(instance.mesh, new Hash(tag)).getHashes());
            }
        }
        ...
        if (request.containsKey("approvees")) {
            for (final String approvee : (List<String>) request.get("approvees")) {
                ...
                approveeTransactions.addAll(TransactionViewModel.fromHash(instance.mesh, new Hash(approvee)).getApprovers(instance.mesh).getHashes());
            }
        }
        ...
        final int milestoneIndex = instance.milestone.latestSolidSubtangleMilestoneIndex;
            ...
            while ((hash = nonAnalyzedTransactions.poll()) != null) {
                ...
                if (analyzedTips.add(hash)) {
                    ...
                    final TransactionViewModel transactionViewModel = TransactionViewModel.fromHash(instance.mesh, hash);
    ...
    private HttpHandler addSecurity(final HttpHandler toWrap) {
        ...
        final List<AuthenticationMechanism> mechanisms = Collections.singletonList(new BasicAuthenticationMechanism("OruMesh Realm"));
