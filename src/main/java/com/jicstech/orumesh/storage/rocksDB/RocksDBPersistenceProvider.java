package com.jicstech.orumesh.storage.rocksDB;
...
import com.jicstech.orumesh.storage.PersistenceProvider;
...
public class RocksDBPersistenceProvider implements PersistenceProvider {
    ...
    public boolean mayExist(Class<?> model, Indexable index) throws Exception {
        return db.keyMayExist(classTreeMap.get().get(model), index.bytes(), new StringBuilder(new StringBuffer()));
    }
