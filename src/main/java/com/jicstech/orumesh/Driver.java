...
import com.jicstech.orumesh.service.API;
...
public class Driver {
    ...
    public static final String MAINNET_NAME = "Driver";
    public static final String TESTNET_NAME = "Driver Testnet";
    public static final String VERSION = "1.1.3.10";
    public static OruMesh oruMesh;
    ...
    public static void main(final String[] args) throws IOException {
        ...
        oruMesh = new OruMesh(configuration);
        ixi = new IXI(oruMesh);
        api = new API(oruMesh, ixi);
        shutdownHook();

        if (configuration.booling(DefaultConfSettings.DEBUG)) {
            log.info("You have set the debug flag");
        }
        ...
        try {
            ...
            log.error("Exception during OruMesh node initialisation: ", e);
            System.exit(-1);
        }
        log.info("OruMesh Node initialised correctly.");
    }
    ...
    private static void validateParams(final Configuration configuration, final String[] args) throws IOException {
        ...
        if (args == null || (args.length < 2 && !configurationInit)) {
            log.error("Invalid arguments list. Provide ini-file 'oruMesh.ini' or API port number (i.e. '-p 14600').");
            printUsage();
        }
        ...
        if (cport == null) {
            log.error("Invalid arguments list. Provide at least the PORT in oruMesh.ini or with -p option");
            printUsage();
        }
        else {
    ...
    private static void shutdownHook() {
        Runtime.getRuntime().addShutdownHook(new Thread(() -> {
            ...
            log.info("Shutting down OruMesh node, please hold tight...");
            try {
                ixi.shutdown();
                api.shutDown();
                oruMesh.shutdown();
            } catch (final Exception e) {
                log.error("Exception occurred shutting down OruMesh node: ", e);
            }
        }, "Shutdown Hook"));
    }
}