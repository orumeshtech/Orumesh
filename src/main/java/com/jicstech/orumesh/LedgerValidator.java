...
import com.jicstech.orumesh.controllers.MilestoneViewModel;
import com.jicstech.orumesh.controllers.StateDiffViewModel;
...
import static com.jicstech.orumesh.Snapshot.latestSnapshotSyncObject;
...
public class LedgerValidator {
    ...
    public LedgerValidator(Mesh mesh, Milestone milestone, TransactionRequester transactionRequester) {
        this.mesh = mesh;
    ...
    private Map<Hash,Long> getLatestDiff(Hash tip, int latestSnapshotIndex, boolean milestone) throws Exception {
        ...
        while ((transactionPointer = nonAnalyzedTransactions.poll()) != null) {
            ...
            if (analyzedTips.add(transactionPointer) && keepScanning) {
                ...
                final TransactionViewModel transactionViewModel = TransactionViewModel.fromHash(mesh, transactionPointer);
    ...
    private void updateSnapshotMilestone(MilestoneViewModel milestone) throws Exception {
        ...
        while ((hashPointer = nonAnalyzedTransactions.poll()) != null) {
            if (visitedHashes.add(hashPointer)) {
                final TransactionViewModel transactionViewModel2 = TransactionViewModel.fromHash(mesh, hashPointer);
                if(transactionViewModel2.snapshotIndex() == 0) {
                    transactionViewModel2.setSnapshot(mesh, index);
     ...
     * It then starts at the earliest consistent milestone index with a confirmed, and analyzes the mesh until it
    ...
    private MilestoneViewModel buildSnapshot(boolean revalidate) throws Exception {
        ...
        synchronized (latestSnapshotSyncObject) {
            ...
            MilestoneViewModel snapshotMilestone = MilestoneViewModel.firstWithSnapshot(mesh);
            while (snapshotMilestone != null) {
                stateDiffViewModel = StateDiffViewModel.load(mesh, snapshotMilestone.getHash());
                ...
                if (!revalidate && updatedSnapshot.isConsistent()) {
                    ...
                    snapshotMilestone = snapshotMilestone.nextWithSnapshot(mesh);
                } else {
                    snapshotMilestone = MilestoneViewModel.first(mesh);
                    do {
                        StateDiffViewModel.load(mesh, snapshotMilestone.getHash()).delete(mesh);
                    } while ((snapshotMilestone = snapshotMilestone.nextWithSnapshot(mesh)) != null);
                    TransactionViewModel transactionViewModel = TransactionViewModel.first(mesh);
                    transactionViewModel.updateSolid(false);
                    transactionViewModel.setSnapshot(mesh, 0);
                    while((transactionViewModel = transactionViewModel.next(mesh)) != null) {
                        transactionViewModel.updateSolid(false);
                        transactionViewModel.setSnapshot(mesh, 0);
                    }
                }
    ...
    public boolean updateSnapshot(MilestoneViewModel milestone) throws Exception {
        TransactionViewModel transactionViewModel = TransactionViewModel.fromHash(mesh, milestone.getHash());
        synchronized (latestSnapshotSyncObject) {
    ...
    public boolean updateFromSnapshot(Hash tip) throws Exception {
        TransactionViewModel transactionViewModel = TransactionViewModel.fromHash(mesh, tip);
